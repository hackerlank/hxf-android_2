...
public class FloderAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(floder.getPhotoList().size()>0)
//        ImageLoader.getInstance().display(floder.getPhotoList().get(0).getPath(), holder.photoIV,
//                mWidth, mWidth);
        com.nostra13.universalimageloader.core.ImageLoader.getInstance().displayImage(StringFinal.IMG_URI_HEAD+floder.getPhotoList().get(0).getPath(), holder.photoIV,new ImageSize(mWidth, mWidth));
        return convertView;
    }
