...
/**
 * Created by 电脑 on 2016/5/21.
 */
public class MyStickerBarView extends StickerBarViewBase {
    /**
     * 布局ID
     */
    public static int getLayoutId() {
        return TuSdkContext.getLayoutResId("my_sticker_bar_view");
    }
    ...
    public interface StickerBarViewDelegate {
         ...
         * @param data
         */
        void onStickerBarViewSelected(MyStickerBarView view, StickerData data);
         ...
         * @param cate
         */
        void onStickerBarViewEmpty(MyStickerBarView view, StickerCategory cate);
    }

    public MyStickerBarView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
    }

    public MyStickerBarView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public MyStickerBarView(Context context) {
        super(context);
    }
     ...
     * 贴纸数据点击事件
     */
    private TuSdkTableView.TuSdkTableViewItemClickDelegate<StickerData, StickerListGrid> mTuSdkTableViewItemClickDelegate = new TuSdkTableView.TuSdkTableViewItemClickDelegate<StickerData, StickerListGrid>() {
        @Override
    ...
    public void loadView() {
        super.loadView();
        mBackGround = this.getViewById(R.id.lsq_sticker_rl_bg);
     ...
     */
    @Override
    protected View buildCateButton(StickerCategory cate, int index, LinearLayout.LayoutParams params) {
    ...
    public void refreshCateDatas(long groupId) {
        if (this.getTableView() == null) return;
        ...
        List<StickerData> datas2 = new ArrayList<>();
        int i = 0;
        for (StickerData stickerData : datas) {
            if (datas.get(i).groupId == groupId) {
                datas2.add(stickerData);
            }
            i++;
        }

        this.getTableView().setModeList(datas2);
        if (this.getTableView().getModeList() != null && this.getTableView().getModeList().size() > 0) {
            this.getTableView().scrollToPosition(0);
        }
        this.getTableView().reloadData();
