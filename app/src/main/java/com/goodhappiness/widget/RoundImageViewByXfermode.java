...
public class RoundImageViewByXfermode extends ImageView {
    ...
    public RoundImageViewByXfermode(Context context, AttributeSet attrs) {
        ...
        mBorderRadius = a.getDimensionPixelSize(
                R.styleable.RoundImageView_radius1, (int) TypedValue
                        .applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                                BODER_RADIUS_DEFAULT, getResources()
                                        .getDisplayMetrics()));// 默认为10dp
        type = a.getInt(R.styleable.RoundImageView_imageType1, TYPE_CIRCLE);// 默认为Circle

        strokeColor = a.getColor(R.styleable.RoundImageView_strokeColor, getResources().getColor(R.color.black));
...
//        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawCircle(getWidth() / 2, getWidth() / 2, getWidth() / 2 - 2,
                p);
        if (null == bitmap || bitmap.isRecycled()) {
            ...
            if (drawable != null) {
                ...
                if (type == TYPE_ROUND) {
                    ...
                    scale = getHeight() * 1.0F / Math.min(dWidth, dHeight);
                }
                //根据缩放比例，设置bounds，相当于缩放图片了
                float f = (float) (dWidth / dHeight);
                if (f > 1.5 || f < 0.66) {
