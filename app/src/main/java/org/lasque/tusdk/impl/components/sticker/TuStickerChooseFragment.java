...
public class TuStickerChooseFragment extends TuStickerChooseFragmentBase implements StickerLocalListFragmentDelegate, TuStickerOnlineFragmentDelegate
{
	...
	{
		super.loadView(view);
		this.getViewById(R.id.iv_back).setOnClickListener(new OnClickListener() {
			@Override
		...
		this.getViewById(R.id.tv_more).setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
	...
	{
		this.setTitle(this.getResString("贴纸库"));
...
//		this.setNavRightButton(this.getResString("lsq_nav_more"), TuSdkContext.getColor("lsq_navigator_button_right_title"));
	...
	protected TuSdkTextButton createCategoryButton(StickerCategory cate)
	{
		...
		int[] colors = new int[] {
				TuSdkContext.getColor("lsq_sticker_title_selected_color"), TuSdkContext.getColor("lsq_999") };
		int[][] states = new int[][] {
