 ...
 * @author Clear
 */
public class TuCameraFragment extends TuCameraFragmentBase
{
	/** 布局ID */
	...
	/**
	 * 相机方向
	 * {@link org.lasque.tusdk.core.utils.hardware.CameraConfigs.CameraFacing}
	 */
	...
	protected void onPermissionGrantedResult(boolean permissionGranted)
	{
		...
		{
			String msg = TuSdkContext.getString("lsq_carema_no_access", ContextUtils.getAppName(getActivity()));
			
			TuSdkViewHelper.alert(permissionAlertDelegate, getActivity(), TuSdkContext.getString("lsq_carema_alert_title"),
	...
	protected void loadAlbumPosterImage()
	{
		...
		if (cameraAlbum != null)
		{
			ArrayList<ImageSqlInfo> photos = ImageSqlHelper.getPhotoList(getActivity(), cameraAlbum.id, true);
		...
		if (takenPhotoFound == false)
		{
			...
			Drawable emptyPoster = TuSdkContext.getDrawable("style_default_camera_album_poster_empty");
	...
	protected void onCameraStateChangedImpl(TuSdkStillCameraInterface camera, CameraState state)
	{
		...
		if (camera.canSupportFlash())
		{
			...
			this.showViewIn(this.getFlashButton(), true);
		}
		else this.showViewIn(this.getFlashButton(), false);
