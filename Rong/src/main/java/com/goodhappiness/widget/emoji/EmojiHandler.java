...
public final class EmojiHandler {
    ...
    public static void addAlphaEmojis(final Context context, final Spannable text, final int emojiSize,float alpha) {
        final int textLength = text.length();
        ...
        for (int i = 0; i < textLength; i += skip) {
            ...
            if (icon > 0) {
                text.setSpan(new EmojiAlphaSpan(context, icon, emojiSize,alpha), i, i + skip, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
    }
